...
def train(output_directory, log_directory, checkpoint_path, warm_start, n_gpus,
    ...
    for epoch in range(epoch_offset, hparams.epochs):
        print('Epoch: %d/%d' % (epoch + 1, hparams.epochs))
        kbar = pkbar.Kbar(target=len(train_loader), width=8)
        for i, batch in enumerate(train_loader):
            ...
            if not overflow and not math.isnan(reduced_loss) and rank == 0:
                ...
                kbar.update(i, values=[("Train-loss", reduced_loss), ("Train-grad norm", grad_norm)])
            ...
            if not overflow and (iteration % hparams.iters_per_checkpoint == 0):
                ...
                kbar.add(1, values=[("Train-loss", reduced_loss), ("Train-grad norm", grad_norm),
                        ("val.loss", val_loss)])
                if rank == 0:
